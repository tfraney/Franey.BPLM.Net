...
public abstract class DefaultFactoryUnit<T, TFunc>(ILogger logger, Lazy<TFunc>? unitOfWork = null, IFactoryUnit<T>? nextDefaultUnit = null) :
    FactoryUnit<T, TFunc>(logger, Verbiage.TypeDefchainfactory, unitOfWork)
    where T : IPacket
    where TFunc : ServiceUnit<T>
{
    ...
    public override async Task<T> ProcessRequestAsync(T packet, ChainStrategyMode strategyMode)
    {
        if (FactoryRule(packet))
        {
                 ...
                 strategyMode == ChainStrategyMode.FullDecoratorChainResponsibility))
            {
                ...
                else packet = ProcessMessage(packet);
            }

            if (packet.Response != null) return packet;
        }

        if (nextDefaultUnit != null) packet = await nextDefaultUnit.ProcessRequestAsync(packet, strategyMode).ConfigureAwait(false);
    ...
    protected override T ProcessMessage(T packet)
    {
